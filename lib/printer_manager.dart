...
class PrinterManager {
  ...
  Future<bool> connect(Printer device,
      {Duration? connectionStabilizationDelay}) async {
  ...
  Future<void> disconnect(Printer device) async {
    if (device.connectionType == ConnectionType.BLUETOOTH_CLASSIC) {
      ...
      try {
        if (device.address != null) {
          await device.disconnect();
          _updateBleConnectionState(device.address!, false,
              fallbackName: device.name);
  ...
  Future<bool> printData(Printer printer, List<int> bytes,
    ...
    if (printer.connectionType == ConnectionType.USB) {
      if (Platform.isWindows) {
        ...
        return using(
          (alloc) => RawPrinter(printer.name!, alloc).printEscPosWin32(bytes),
        );
      } else {
  ...
  void _updateOrAddPrinter(Printer printer) {
    ...
    if (printer.connectionType == ConnectionType.BLE &&
      ...
      _ensureBleConnectionListener(printer.address!,
  ...
  void _ensureBleConnectionListener(String deviceId, {String? fallbackName}) {
    ...
    _bleConnectionSubscriptions[deviceId] =
      ...
      (isConnected) {
        _updateBleConnectionState(deviceId, isConnected,
  ...
  void _updateBleConnectionState(String deviceId, bool isConnected,
      {String? fallbackName}) {
    final index = _devices.indexWhere(
      (device) =>
          device.connectionType == ConnectionType.BLE &&
    ...
    _devices[index] =
        current.copyWith(isConnected: isConnected, name: resolvedName);
    sortDevices();
  }
